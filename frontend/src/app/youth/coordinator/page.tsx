...
import {
  ...
  ChevronDown, ChevronUp, Pin, AlertCircle, Clock,
  MapPin, UserCheck, UserX, Loader2, RefreshCw,
} from 'lucide-react'
...
function Toast({ msg, ok, onClose }: { msg: string; ok: boolean; onClose: () => void }) {
  ...
  return (
    <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 px-5 py-3 rounded-2xl shadow-2xl text-white text-sm font-medium transition-all ${ok ? 'bg-green-600' : 'bg-red-600'}`}>
...
export default function YouthLeaderDashboard() {
  ...
  // Pre-fill attendance entries when members load
  useEffect(() => {
  ...
  async function handleAddMember(e: React.FormEvent) {
    ...
    try {
      ...
      const m = await listMembers(DEPT); setMembers(m)
    } catch (err: unknown) {
  ...
  async function handleToggleActive(m: DeptMember) {
    try {
      ...
      const updated = await listMembers(DEPT); setMembers(updated)
    } catch (err: unknown) {
      toast$((err as Error).message || 'Failed to update member', false)
    }
  }

  async function handleRemoveMember(m: DeptMember) {
    if (!confirm(`Remove ${m.name} from Youth?`)) return
    try {
      await removeMember(DEPT, m.user_id)
      toast$('Member removed')
      const updated = await listMembers(DEPT); setMembers(updated)
    } catch (err: unknown) {
      toast$((err as Error).message || 'Failed to remove member', false)
    }
  }
  ...
  async function handlePostAnnouncement(e: React.FormEvent) {
    ...
    try {
      ...
      const a = await listAnnouncements(DEPT); setAnnouncements(a)
    } catch (err: unknown) {
      toast$((err as Error).message || 'Failed to post announcement', false)
  ...
  async function handlePostActivity(e: React.FormEvent) {
    ...
    try {
      ...
      const ac = await listActivities(DEPT); setActivities(ac)
    } catch (err: unknown) {
  ...
  async function handleSaveAttendance(e: React.FormEvent) {
    e.preventDefault()
    if (!sessionLabel.trim() || !sessionDate) { toast$('Session label and date are required', false); return }
    setSavingAtt(true)
    try {
      ...
      const att = await listAttendance(DEPT); setAttendance(att)
    } catch (err: unknown) {
  ...
  const filteredAtt = attendance.filter(r =>
    !attFilter || r.session_label.toLowerCase().includes(attFilter.toLowerCase()) ||
  ...
  return (
    <div className="min-h-screen bg-amber-50 font-sans">
      {/* Header */}
      <header className="bg-amber-600 text-white px-4 py-4 flex items-center justify-between shadow-lg">
        <div>
          <div className="text-xs font-semibold uppercase tracking-widest opacity-80">Youth Ministry</div>
          <h1 className="text-lg font-bold">Leader Dashboard</h1>
      ...
      {/* Tab bar */}
      <nav className="bg-white border-b border-amber-100 flex overflow-x-auto">
        {TABS.map(t => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
        ...
        {tab === 'overview' && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                { label: 'Total Members', value: stats?.total_members ?? '—', color: 'bg-amber-500' },
                { label: 'Announcements', value: stats?.total_announcements ?? '—', color: 'bg-orange-500' },
                { label: 'Activities', value: stats?.total_activities ?? '—', color: 'bg-yellow-500' },
              ].map(s => (
            ...
            <section className="bg-white rounded-2xl p-4 shadow-sm">
              <h2 className="font-semibold text-gray-700 mb-3 flex items-center gap-2">
                <Bell size={16} className="text-amber-500" /> Recent Notices
              </h2>
              {announcements.length === 0 ? (
                <p className="text-gray-400 text-sm text-center py-4">No announcements yet</p>
              ) : announcements.slice(0, 3).map(a => (
                <div key={a.id} className="border-l-4 border-amber-400 pl-3 py-2 mb-2">
            ...
            <section className="bg-white rounded-2xl p-4 shadow-sm">
              <h2 className="font-semibold text-gray-700 mb-3 flex items-center gap-2">
                <Calendar size={16} className="text-amber-500" /> Upcoming Activities
              </h2>
              {activities.length === 0 ? (
                <p className="text-gray-400 text-sm text-center py-4">No activities scheduled</p>
              ) : activities.slice(0, 3).map(a => (
                <div key={a.id} className="flex items-start gap-3 py-2 border-b border-gray-50 last:border-0">
                  <div className="bg-amber-100 rounded-xl p-2"><Calendar size={14} className="text-amber-600" /></div>
                  <div>
            ...
            <section className="bg-white rounded-2xl p-4 shadow-sm">
              <h2 className="font-semibold text-gray-700 mb-3 flex items-center gap-2">
                <Users size={16} className="text-amber-500" /> Active Members ({activeMembers.length})
              </h2>
              ...
              {activeMembers.length > 5 && (
                <button onClick={() => setTab('members')} className="text-amber-600 text-xs mt-2 hover:underline">
          ...
          <div className="space-y-4">
            ...
            <section className="bg-white rounded-2xl p-4 shadow-sm">
              ...
              <form onSubmit={handleAddMember} className="space-y-3">
                ...
                <button type="submit" disabled={addingMember}
                  className="w-full bg-amber-600 text-white rounded-xl py-2 text-sm font-medium flex items-center justify-center gap-2 hover:bg-amber-700 disabled:opacity-50">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-white border border-gray-200 rounded-xl pl-9 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400"
              />
            </div>
              ...
              {filteredMembers.length === 0 ? (
                ...
                <div key={m.id} className="flex items-center gap-3 p-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {m.role_label && <div className="text-xs text-amber-600 mt-0.5">{m.role_label}</div>}
                  </div>
            ...
            <section className="bg-white rounded-2xl p-4 shadow-sm">
              ...
              <form onSubmit={handlePostAnnouncement} className="space-y-3">
                ...
                <div className="flex items-center gap-4 text-sm">
                  ...
                  <label className="flex items-center gap-1.5 cursor-pointer">
                    <input type="checkbox" checked={annPinned} onChange={e => setAnnPinned(e.target.checked)} className="accent-amber-500" />
            ...
            <section className="bg-white rounded-2xl p-4 shadow-sm">
              ...
              <form onSubmit={handleSaveAttendance} className="space-y-3">
                <div className="grid grid-cols-2 gap-2">
                  <input
                    type="text" required
                    placeholder="Session label (e.g. Sunday Meeting)"
                    value={sessionLabel} onChange={e => setSessionLabel(e.target.value)}
                    className="col-span-2 border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400"
                  />
                  <input
                    type="date" required
                ...
                {activeMembers.length === 0 ? (
                  ...
                  <div className="border border-gray-100 rounded-xl overflow-hidden">
                    ...
                    {activeMembers.map(m => (
                      <label key={m.user_id} className="flex items-center gap-3 px-3 py-2.5 border-b border-gray-50 last:border-0 cursor-pointer hover:bg-amber-50">
                        <input
                          type="checkbox"
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="border border-gray-200 rounded-xl pl-8 pr-3 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-amber-400 w-36"
                  />
                </div>
              </div>
