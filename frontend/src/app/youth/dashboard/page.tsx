...
export default function YouthMemberDashboard() {
  ...
  return (
    <div className="min-h-screen bg-amber-50 font-sans flex flex-col">
      {/* Header */}
      <header className="bg-amber-600 text-white px-4 pt-10 pb-6 relative">
        <button onClick={signOut} className="absolute top-4 right-4 p-2 rounded-full bg-white/20 hover:bg-white/30">
          <LogOut size={16} />
        </button>
        <div className="text-xs font-semibold uppercase tracking-widest opacity-75">Youth Ministry</div>
        <h1 className="text-2xl font-bold mt-1">Hello, {user?.name?.split(' ')[0] ?? 'Member'} 👋</h1>
        <div className="flex items-center gap-2 mt-1.5">
          {isMember
            ? <span className="text-xs bg-white/20 px-2.5 py-0.5 rounded-full flex items-center gap-1"><Users size={11} /> Youth Member</span>
      ...
      {loadingData && <div className="h-1 bg-amber-200"><div className="h-full bg-amber-600 animate-pulse w-1/2 rounded-full" /></div>}
      ...
      <main className="flex-1 overflow-y-auto px-4 py-4 pb-24 space-y-4 max-w-lg mx-auto w-full">
        ...
        {tab === 'home' && (
          <div className="space-y-4">
            {/* Join card if not yet a member */}
            {!isMember && (
              <div className="bg-white rounded-2xl p-5 shadow-sm border-l-4 border-amber-400">
                <h2 className="font-bold text-gray-800 mb-1 flex items-center gap-2"><UserPlus size={16} className="text-amber-500" /> Join Youth Ministry</h2>
                <p className="text-sm text-gray-500 mb-3">Tap below to officially register as a Youth Ministry member. You'll get access to group chat, attendance tracking, and all ministry updates.</p>
                {joinMsg && <p className="text-sm text-green-600 mb-2 font-medium">{joinMsg}</p>}
                <button onClick={handleJoin} disabled={joining}
                  className="w-full bg-amber-600 text-white rounded-xl py-2.5 text-sm font-semibold flex items-center justify-center gap-2 hover:bg-amber-700 disabled:opacity-50">
            ...
            {isMember && pct !== null && (
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-amber-600 text-white rounded-2xl p-3 text-center shadow">
            ...
            {announcements.length > 0 && (
              <section className="bg-white rounded-2xl p-4 shadow-sm">
                <h2 className="text-sm font-semibold text-gray-600 mb-2 flex items-center gap-1.5"><Bell size={14} className="text-amber-500" /> Latest Notice</h2>
                <div className={`border-l-4 pl-3 ${announcements[0].is_urgent ? 'border-red-400' : 'border-amber-400'}`}>
                ...
                {announcements.length > 1 && (
                  <button onClick={() => setTab('notices')} className="text-amber-600 text-xs mt-2 hover:underline">View all {announcements.length} notices →</button>
                )}
              </section>
            )}
            ...
            {upcomingActivities.length > 0 && (
              <section className="bg-white rounded-2xl p-4 shadow-sm">
                <h2 className="text-sm font-semibold text-gray-600 mb-2 flex items-center gap-1.5"><Calendar size={14} className="text-amber-500" /> Next Activity</h2>
                <div className="flex items-start gap-3">
                  <div className="bg-amber-100 rounded-xl p-2 flex-shrink-0"><Calendar size={18} className="text-amber-600" /></div>
                  <div>
            ...
            {announcements.length === 0 && activities.length === 0 && !loadingData && isMember && (
              <div className="bg-white rounded-2xl p-8 text-center shadow-sm">
                ...
                <p className="text-gray-400 text-xs mt-1">Your leader will post updates here</p>
              </div>
            )}
          </div>
        )}
        ...
        {tab === 'notices' && (
          <div className="space-y-3">
            ...
            {sortedAnn.length === 0
              ...
              : sortedAnn.map(a => (
                <div key={a.id} className={`bg-white rounded-2xl p-4 shadow-sm border-l-4 ${a.is_urgent ? 'border-red-400' : a.is_pinned ? 'border-amber-400' : 'border-gray-100'}`}>
            ...
            {activities.length === 0
              ...
              : activities.map(a => (
                <div key={a.id} className="bg-white rounded-2xl p-4 shadow-sm">
                  <span className="text-xs uppercase tracking-wide text-amber-600 font-semibold">{a.activity_type}</span>
        ...
        {tab === 'attendance' && (
          <div className="space-y-4">
            ...
            {!isMember
              ? <div className="bg-white rounded-2xl p-8 text-center shadow-sm"><UserPlus size={32} className="mx-auto text-amber-300 mb-2" /><p className="text-gray-400 text-sm">Join the ministry to track attendance</p><button onClick={() => setTab('home')} className="mt-3 text-amber-600 text-sm font-medium hover:underline">Go to Home to join →</button></div>
              : attendance.length === 0
              ...
              : <>
                  {pct !== null && (
                    <div className="bg-amber-600 text-white rounded-2xl p-4 flex items-center justify-between shadow">
        ...
        {tab === 'chat' && (
          <div className="flex flex-col" style={{ height: 'calc(100vh - 200px)' }}>
            <h2 className="font-semibold text-gray-700 mb-3 flex items-center gap-2"><MessageSquare size={16} className="text-amber-500" /> Youth Group Chat</h2>

            {!isMember ? (
              <div className="bg-white rounded-2xl p-8 text-center shadow-sm flex-1">
                <MessageSquare size={32} className="mx-auto text-amber-300 mb-2" />
                <p className="text-gray-500 text-sm font-medium">Members only</p>
                <p className="text-gray-400 text-xs mt-1">Join the Youth Ministry to access group chat</p>
                <button onClick={() => setTab('home')} className="mt-3 text-amber-600 text-sm font-medium hover:underline">Join from Home tab →</button>
              </div>
            ) : (
              <div className="flex flex-col flex-1 bg-white rounded-2xl shadow-sm overflow-hidden">
                    ...
                    : messages.map(m => (
                      <div key={m.id} className={`flex ${m.is_mine ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[75%] ${m.is_mine ? 'items-end' : 'items-start'} flex flex-col gap-0.5`}>
                          ...
                          <div className={`px-3 py-2 rounded-2xl text-sm ${m.is_mine ? 'bg-amber-500 text-white rounded-br-sm' : 'bg-gray-100 text-gray-800 rounded-bl-sm'}`}>
                            {m.content}
                          </div>
                ...
                <form onSubmit={handleSendMessage} className="border-t border-gray-100 p-3 flex items-center gap-2">
                  ...
                  <button type="submit" disabled={sending || !chatInput.trim()}
                    className="bg-amber-600 text-white p-2 rounded-xl hover:bg-amber-700 disabled:opacity-40">
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 flex shadow-lg">
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`flex-1 flex flex-col items-center py-2 gap-0.5 transition-colors ${tab === t.id ? 'text-amber-600' : 'text-gray-400 hover:text-amber-500'}`}>
            {t.icon}
