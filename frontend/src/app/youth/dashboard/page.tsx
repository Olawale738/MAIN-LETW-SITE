  ...
  return (
    <div className="min-h-screen bg-amber-50 font-sans flex flex-col">
      ...
      <header className="bg-amber-600 text-white px-4 pt-10 pb-6 relative">
        ...
        <p className="text-sm opacity-75 mt-0.5">Welcome to the Youth dashboard</p>
      </header>

      {/* Loading bar */}
      {loadingData && (
        <div className="h-1 bg-amber-200">
          <div className="h-full bg-amber-600 animate-pulse w-1/2 rounded-full" />
        </div>
      )}
        ...
        {tab === 'home' && (
          <div className="space-y-4">
            ...
            {announcements.length > 0 && (
              <section className="bg-white rounded-2xl p-4 shadow-sm">
                <h2 className="text-sm font-semibold text-gray-600 mb-2 flex items-center gap-1.5">
                  <Bell size={14} className="text-amber-500" /> Latest Notice
                </h2>
                ...
                {announcements.length > 1 && (
                  <button onClick={() => setTab('notices')} className="text-amber-600 text-xs mt-2 hover:underline">
            ...
            {upcomingActivities.length > 0 && (
              <section className="bg-white rounded-2xl p-4 shadow-sm">
                <h2 className="text-sm font-semibold text-gray-600 mb-2 flex items-center gap-1.5">
                  <Calendar size={14} className="text-amber-500" /> Next Activity
                </h2>
        ...
        {TABS.map(t => (
          <button
            ...
            className={`flex-1 flex flex-col items-center py-2 gap-0.5 transition-colors ${
              tab === t.id ? 'text-amber-600' : 'text-gray-400 hover:text-amber-500'
            }`}
          >
            {t.icon}
