...
export default function BlogPostPage() {
    ...
    return (
        <main className="min-h-screen bg-[#fbf5e6]">
            ...
            {/* Only overlap the hero image when one exists — otherwise pad down
                so the card clears the fixed navbar instead of hiding under it. */}
            <article className={`max-w-3xl mx-auto px-4 sm:px-6 relative z-10 pb-24 ${hasHero ? '-mt-32' : 'pt-28 sm:pt-32'}`}>
                <div className="bg-white rounded-3xl shadow-2xl p-6 sm:p-10">
                    ...
                    <div className="flex items-center gap-4 text-xs text-gray-500 mt-4 flex-wrap">
                        ...
                        {post.tags && <span className="inline-flex gap-1.5 flex-wrap">{post.tags.split(',').map(t => <span key={t} className="bg-[#f5bb00]/15 text-[#140152] px-2 py-0.5 rounded font-bold">{t.trim()}</span>)}</span>}
                    </div>
                    ...
                    {/* Closing CTA so an image-less / short post doesn't trail off into blank space */}
